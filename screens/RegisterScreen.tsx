...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Alert, ScrollView, KeyboardAvoidingView, Platform, Animated, ActivityIndicator, ImageBackground, StatusBar } from 'react-native';
...
export default function RegisterScreen({ onRegisterSuccess, onBack }: Props) {
    ...
    const validarSenha = (password: string): string | null => {
        ...
        if (!allowedCharsRegex.test(password)) return "Contém caracteres inválidos.";
        const specialCharCount = (password.match(/[!@#$%&*?]/g) || []).length;
        if (specialCharCount !== 2) return "A senha deve conter exatamente 2 símbolos.";
        if (!(/[a-zA-Z]/.test(password) && /\d/.test(password))) return "A senha deve conter letra e número.";
        return null; 
    };
        ...
        return (
            <View style={styles.successContainer}>
                <Animated.View style={[styles.successContent, { opacity: fadeAnim }]}>
                    <View style={styles.successIconCircle}><Text style={styles.successCheckMark}>✓</Text></View>
                    ...
                    <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                        <View style={styles.cardContainer}>
                            ...
                            <View style={styles.rowContainer}>
                                ...
                                <View style={[styles.inputGroup, { flex: 1, marginLeft: 8 }]}><Text style={styles.label}>Número</Text><TextInput style={styles.input} placeholder="123" placeholderTextColor="#aaa" value={numero} onChangeText={setNumero} editable={!loading}/></View>
                            </View>
                            ...
                            <View style={styles.rowContainer}>
                                <View style={[styles.inputGroup, { flex: 1, marginRight: 8 }]}><Text style={styles.label}>Senha</Text><TextInput style={styles.input} placeholder="******" placeholderTextColor="#aaa" value={senha} onChangeText={setSenha} secureTextEntry editable={!loading}/><Text style={styles.tipText}>Máx. 11, letras, nums e 2 símbolos.</Text></View>
                            ...
                            <TouchableOpacity style={styles.backButton} onPress={onBack} disabled={loading}><Text style={styles.backText}>Já possui conta? <Text style={styles.backTextBold}>Voltar</Text></Text></TouchableOpacity>
                        </View>
                    </ScrollView>
...
const styles = StyleSheet.create({
    ...
    tipText: { fontSize: 11, color: '#ff6600', marginTop: 5, marginLeft: 2 },
    botao: { backgroundColor: '#ff6600', paddingVertical: 16, borderRadius: 12, alignItems: 'center', marginTop: 20, shadowColor: '#ff6600', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 5, elevation: 4, ...Platform.select({ web: { cursor: 'pointer' } as any }) },
